...
function useHover(stopPropagation = true) {
  ...
  function EndPoint({ position, onDrag, onEnd }: any) {
    ...
    return (
      <mesh position={position} {...bindDrag} {...bindHover as Object}>
...
function Controls({ children }: any) {
  ...
  return (
    <>
      <orbitControls ref={ref} args={[camera, gl.domElement]} enableDamping enabled={api[0]} enableZoom={false} enableRotate={false} />
      <camContext.Provider value={api as any}>{children}</camContext.Provider>
    </>
  )
}

export { Line, Controls }